...
# define the 1st convolution layer. use border_mode= and input_shape only on first layer 
# border_model=value restricts convolution to only where the input and the filter fully overlap (ie. not partial overlap)
...
model.add(Flatten())
model.add(Dense(128))   # Fully connected layer in Ker 
model.add(Activation('relu'))
